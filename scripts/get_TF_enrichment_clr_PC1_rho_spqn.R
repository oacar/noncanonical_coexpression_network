...
#coexpression network method used is clr + remove top PC (batch correction) + rho + spqn
...
rho<-readRDS('spqn_raw5_sample400_clr_pc1.RDS')

num_obs<-readRDS('numobs_raw5_sample400.RDS')
all(rownames(rho)==rownames(num_obs))
all(colnames(rho)==colnames(num_obs))
rho[num_obs<400]<-NA
