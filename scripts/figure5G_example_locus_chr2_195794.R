#script to plot figure 5G: an example locus where there is no terminator present 
...
gff_denovo<-filter(orf_info, transcript == 'chr2_195794')
gff_denovo$title<-'denovo'

gff_conserved<-filter(orf_info, transcript== 'chr2_194122')
...
preads1<-ggplot(tif)+ geom_segment(mapping=aes(x=coor1, xend=coor2, y=position,yend=position),color="#404142")+theme_minimal()+ 
  scale_x_continuous(limits=c(pc1-1, pc2+1),label=comma)+coord_cartesian(xlim=c(pc1-1, pc2+1), expand=F)+
  labs(y="TIF-seq reads",x=NULL)+theme(axis.text.y = element_blank(), axis.text.x=element_blank(), axis.title.y = element_text(size=axis_title_size))
...
pdf('./20230106_figures/loci_chr2_195794.pdf',width=4,height=3.5)
